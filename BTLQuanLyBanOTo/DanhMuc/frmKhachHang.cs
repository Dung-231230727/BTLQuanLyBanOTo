...
namespace BTLQuanLyBanOTo.DanhMuc
{
    public partial class frmKhachHang : Form
    {
        ...
        private string TaoMaTuDong()
        {
            ...
            string sql = "SELECT MAX(MaKhach) FROM KhachHang WHERE MaKhach LIKE @prefix";
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (TrangThai_HienTai == 1)
            {
                ...
                MessageBox.Show("Khách hàng đang hoạt động (Trạng thái = 1). Vui lòng chuyển sang trạng thái 0 (Sửa -> Ngừng hoạt động) trước khi xóa vĩnh viễn.", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            btnThem.Enabled = false;
            btnSua.Enabled = false;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action == "add")
                {
                    ...
                    string sql = @"insert into KhachHang(MaKhach, TenKhach, DienThoai, DiaChi, TrangThai)
                                 values(@ma, @ten, @dt, @dc, @tt)";
                ...
                if (action == "edit")
                {
                    string sql = "update KhachHang set TenKhach = @ten, DienThoai = @dt, DiaChi = @dc, TrangThai=@tt where MaKhach = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim()),
                        new SqlParameter("@ten", txtTen.Text.Trim()),
