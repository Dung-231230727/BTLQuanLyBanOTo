    ...
    public partial class frmNhaCungCap : Form
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (TrangThai_HienTai == 1)
            {
                ...
                MessageBox.Show("Nhà cung cấp đang hoạt động (Trạng thái = 1). Vui lòng chuyển sang trạng thái 0 (Sửa -> Ngừng hoạt động) trước khi xóa vĩnh viễn.", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            btnThem.Enabled = false;
            btnSua.Enabled = false;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action == "edit")
                {
                    string sql = "update NhaCungCap set TenNCC = @ten, DiaChi = @dc, DienThoai = @dt, TrangThai=@tt where MaNCC = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim()),
                        new SqlParameter("@ten", txtTen.Text.Trim()),
