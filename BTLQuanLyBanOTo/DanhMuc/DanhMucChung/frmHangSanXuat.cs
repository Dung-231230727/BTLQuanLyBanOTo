    ...
    public partial class frmHangSanXuat : Form
    {
        ...
        public void LoadDGV()
        {
            try
            {
                ...
                dgvHangSX.DataSource = dt.ExecuteQuery(sql);
            }
            catch (Exception ex)
            {
        ...
        private string TaoMaTuDong()
        {
            ...
            string sql = "SELECT MAX(MaHangSX) FROM HangSX WHERE MaHangSX LIKE @prefix";
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (TrangThai_HienTai == 1)
            {
                ...
                MessageBox.Show("Hãng sản xuất đang được sử dụng (Trạng thái = 1). Vui lòng chuyển sang trạng thái 0 (Sửa -> Ngừng hoạt động) trước khi xóa vĩnh viễn.", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            btnThem.Enabled = false;
            btnSua.Enabled = false;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action == "add")
                {
                    string sqlKT = "select count(*) from HangSX where MaHangSX = @ma";
                    ...
                    if (rKT > 0)
                    {
                        MessageBox.Show("Đã có mã hãng sản xuất này. Vui lòng tạo mã khác!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    string sql = "insert into HangSX(MaHangSX,TenHangSX,TrangThai) values(@ma,@ten,@tt)";
                ...
                if (action == "edit")
                {
                    string sql = "update HangSX set TenHangSX = @ten, TrangThai=@tt where MaHangSX = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim()),
                        new SqlParameter("@ten", txtTen.Text.Trim()),
                ...
                if (action == "delete")
                {
                    ...
                    string sql = "delete from HangSX where MaHangSX = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim())
                    });
            ...
            {
                if (ex.Number == 547)
                    MessageBox.Show("Không thể xóa vì hãng sản xuất này đang được sử dụng ở bảng khác!", "Lỗi ràng buộc", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
        ...
        private void dgvHangSX_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                var row = dgvHangSX.Rows[e.RowIndex];
                txtMa.Text = row.Cells["MaHangSX"].Value?.ToString();
                txtTen.Text = row.Cells["TenHangSX"].Value?.ToString();
