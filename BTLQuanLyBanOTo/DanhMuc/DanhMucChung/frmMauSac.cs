    ...
    public partial class frmMauSac : Form
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (TrangThai_HienTai == 1)
            {
                ...
                MessageBox.Show("Màu sắc đang được sử dụng (Trạng thái = 1). Vui lòng chuyển sang trạng thái 0 (Sửa -> Ngừng hoạt động) trước khi xóa vĩnh viễn.", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            btnThem.Enabled = false;
            btnSua.Enabled = false;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action == "add")
                {
                    ...
                    string sql = "insert into MauSac(MaMau,TenMau,TrangThai) values(@ma,@ten,@tt)";
                ...
                if (action == "edit")
                {
                    string sql = "update MauSac set TenMau = @ten, TrangThai=@tt where MaMau = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim()),
                        new SqlParameter("@ten", txtTen.Text.Trim()),
