...
namespace BTLQuanLyBanOTo.DanhMuc.DanhMucChung
{
    public partial class frmSoChoNgoi : Form
    {
        ...
        private string TaoMaTuDong()
        {
            ...
            string sql = "SELECT MAX(MaSoCho) FROM SoChoNgoi WHERE MaSoCho LIKE @prefix";
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action == "add")
                {
                    string sqlKT = "select count(*) from SoChoNgoi where MaSCN = @ma";
                    ...
                    if (rKT > 0)
                    {
                        MessageBox.Show("Đã có mã số chỗ ngồi này. Vui lòng tạo mã khác!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                ...
                if (action == "delete")
                {
                    ...
                    string sql = "delete from SoChoNgoi where MaSCN = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim())
                    });
            ...
            {
                if (ex.Number == 547)
                    MessageBox.Show("Không thể xóa vì số chỗ ngồi này đang được sử dụng ở bảng khác!", "Lỗi ràng buộc", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
