...
namespace BTLQuanLyBanOTo.DanhMuc.DanhMucChung
{
    public partial class frmNuocSanXuat : Form
    {
        ...
        private string TaoMaTuDong()
        {
            ...
            string sql = "SELECT MAX(MaNuocSX) FROM NuocSX WHERE MaNuocSX LIKE @prefix";
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (TrangThai_HienTai == 1)
            {
                ...
                MessageBox.Show("Nước sản xuất đang được sử dụng (Trạng thái = 1). Vui lòng chuyển sang trạng thái 0 (Sửa -> Ngừng hoạt động) trước khi xóa vĩnh viễn.", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            btnThem.Enabled = false;
            btnSua.Enabled = false;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action == "add")
                {
                    ...
                    if (rKT > 0)
                    {
                        MessageBox.Show("Đã có mã nước sản xuất này. Vui lòng tạo mã khác!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    string sql = "insert into NuocSX(MaNuocSX,TenNuocSX,TrangThai) values(@ma,@ten,@tt)";
                ...
                if (action == "edit")
                {
                    string sql = "update NuocSX set TenNuocSX = @ten, TrangThai=@tt where MaNuocSX = @ma";
                    int r = dt.ExecuteNonQuery(sql, new SqlParameter[]
                    {
                        new SqlParameter("@ma", txtMa.Text.Trim()),
                        new SqlParameter("@ten", txtTen.Text.Trim()),
            ...
            {
                if (ex.Number == 547)
                    MessageBox.Show("Không thể xóa vì nước sản xuất này đang được sử dụng ở bảng khác!", "Lỗi ràng buộc", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
